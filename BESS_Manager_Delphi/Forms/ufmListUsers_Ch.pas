...
uses
  ...
  Vcl.ExtCtrls, MySkinWhite, dxBarBuiltInMenu, cxPC, dxSkinOffice2013White, System.Hash;

const
  mCaption = 'Користувач: ';

type
  TfmListUsers_Ch = class(TfmBaseObjectForm)
...
var
  fmListUsers_Ch: TfmListUsers_Ch;
  OwnSender: TObject;
...
constructor TfmListUsers_Ch.Create(AOwner: TComponent; Param: TStructure; Sender: TObject; AFormStyle: TFormStyle);
begin
...
function TfmListUsers_Ch.CheckDataForErrors(): Boolean;
begin
  Result := false;
...
procedure TfmListUsers_Ch.aSaveExecute(Sender: TObject);
begin
  ...
  try
    ...
    spSaveForm.Params.ParamByName('USER_NAME').AsString := spDataField.FieldValues['NAME'];
    spSaveForm.Params.ParamByName('DEL').AsBoolean      := false;
    spSaveForm.Params.ParamByName('USER_PASSWORD').AsString :=
      LowerCase(THashSHA2.GetHashString(spDataField.FieldValues['USER_PASSWORD']));
  ...
  if Assigned(Owner) and (Owner is TfmListUsers) then
    (Owner as TfmListUsers).spList.Refresh;
end;

procedure TfmListUsers_Ch.aConfirmExecute(Sender: TObject);
begin
  aSaveExecute(nil);
  if FormCanClose then Close;
end;

procedure TfmListUsers_Ch.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := TCloseAction.caFree;
end;

procedure TfmListUsers_Ch.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
