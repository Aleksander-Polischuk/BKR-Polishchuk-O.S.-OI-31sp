...
type
  TfmListUsers = class(TfmBaseListForm)
    gList: TcxGrid;
...
var
  fmListUsers    : TfmListUsers;
  OwnSender  : TObject;

implementation

Uses ufmListUsers_Ch, HomePage;
...
constructor TfmListUsers.Create(AOwner: TComponent; Param: TStructure; Sender: TObject; AFormStyle: TFormStyle);
begin
...
procedure TfmListUsers.aAddNewExecute(Sender: TObject);
begin
  var Param := TStructure.Create;
  try
    Param.AddParam('Operation', FormOperation.Add);

    mainModule.GetForm(TTypeForm.FormObj, DBTableName.SYS_USERS, Param, Self, Sender, fsStayOnTop);
  finally
    Param.Free;
  end;
end;

procedure TfmListUsers.aClearFindExecute(Sender: TObject);
begin
  edFind.Text := '';
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel).ApplyText('');
end;

procedure TfmListUsers.aDeleteExecute(Sender: TObject);
var
  ID: Integer;
begin
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'SYS_USERS', ID, false, true) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmListUsers.aEditExecute(Sender: TObject);
begin
  if spList.RecordCount = 0 then exit;
...
procedure TfmListUsers.aMarkDeleteExecute(Sender: TObject);
var
  ID: Integer;
  MarkDel: Boolean;
begin
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'SYS_USERS', ID, MarkDel, false) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmListUsers.aRefreshExecute(Sender: TObject);
begin
  spList.Refresh;
end;

procedure TfmListUsers.aSelectExecute(Sender: TObject);
begin
...
procedure TfmListUsers.edFindPropertiesEditValueChanged(Sender: TObject);
begin
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel)
    .ApplyText(edFind.Text);
end;

procedure TfmListUsers.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmListUsers.gListDBTableViewDblClick(Sender: TObject);
begin
  aSelect.Execute;
end;

procedure TfmListUsers.gListDBTableViewStylesGetContentStyle(Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
  var AStyle: TcxStyle);
begin
