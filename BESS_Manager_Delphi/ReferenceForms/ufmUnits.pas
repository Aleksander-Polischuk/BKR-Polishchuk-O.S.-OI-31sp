...
type
  TfmUnits = class(TfmBaseListForm)
    gList: TcxGrid;
...
constructor TfmUnits.Create(AOwner: TComponent; Param: TStructure; Sender: TObject; AFormStyle: TFormStyle);
begin
...
procedure TfmUnits.aAddNewCopyExecute(Sender: TObject);
begin
  ...
  try
    ...
    mainModule.GetForm(TTypeForm.FormObj, DBTableName.REF_UNITS, Param, Self, Sender, fsStayOnTop);
  finally
    Param.Free;
  end;
end;

procedure TfmUnits.aAddNewExecute(Sender: TObject);
begin
  var Param := TStructure.Create;
  try
...
procedure TfmUnits.aClearFindExecute(Sender: TObject);
begin
  edFind.Text := '';
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel).ApplyText('');
end;

procedure TfmUnits.aDeleteExecute(Sender: TObject);
var
  ID: Integer;
begin
  if spList.RecordCount = 0 then exit;
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_UNITS', ID, false, true) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmUnits.aEditExecute(Sender: TObject);
begin
  if spList.RecordCount = 0 then exit;
...
procedure TfmUnits.aMarkDeleteExecute(Sender: TObject);
var
  ID: Integer;
  MarkDel: Boolean;
begin
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_UNITS', ID, MarkDel, false) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmUnits.aRefreshExecute(Sender: TObject);
begin
  spList.Refresh;
end;

procedure TfmUnits.aSelectExecute(Sender: TObject);
begin
...
procedure TfmUnits.edFindPropertiesEditValueChanged(Sender: TObject);
begin
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel)
    .ApplyText(edFind.Text);
end;

procedure TfmUnits.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmUnits.gListDBTableViewDblClick(Sender: TObject);
begin
  aSelect.Execute;
end;

procedure TfmUnits.gListDBTableViewKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  Case Key of
...
procedure TfmUnits.gListDBTableViewStylesGetContentStyle(Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
  var AStyle: TcxStyle);
begin
