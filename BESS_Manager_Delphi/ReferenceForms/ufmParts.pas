...
uses
  ...
  cxClasses, cxGridCustomView, cxGrid, MySkinWhite, cxCurrencyEdit, dxSkinOffice2013White, Vcl.ComCtrls;

type
  TfmParts = class(TfmBaseListForm)
    spList: TFDQuery;
    dsList: TDataSource;
    ...
    gListDBTableViewAREA: TcxGridDBColumn;
...
constructor TfmParts.Create(AOwner: TComponent; Param: TStructure; Sender: TObject; AFormStyle: TFormStyle);
begin
...
procedure TfmParts.aAddNewCopyExecute(Sender: TObject);
begin
  ...
  try
    ...
    mainModule.GetForm(TTypeForm.FormObj, DBTableName.REF_PARTS, Param, Self, Sender, fsStayOnTop);
  finally
    Param.Free;
  end;
end;

procedure TfmParts.aAddNewExecute(Sender: TObject);
begin
  var Param := TStructure.Create;
  try
...
procedure TfmParts.aClearFindExecute(Sender: TObject);
begin
  edFind.Text := '';
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel).ApplyText('');
end;

procedure TfmParts.aDeleteExecute(Sender: TObject);
var
  ID: Integer;
begin
  if spList.RecordCount = 0 then exit;
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_PARTS', ID, false, true) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmParts.aEditExecute(Sender: TObject);
begin
  if spList.RecordCount = 0 then exit;
...
procedure TfmParts.aMarkDeleteExecute(Sender: TObject);
var
  ID: Integer;
  MarkDel: Boolean;
begin
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_PARTS', ID, MarkDel, false) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmParts.aRefreshExecute(Sender: TObject);
begin
  spList.Refresh;
end;

procedure TfmParts.aSelectExecute(Sender: TObject);
begin
...
procedure TfmParts.edFindPropertiesEditValueChanged(Sender: TObject);
begin
...
procedure TfmParts.gListDBTableViewDblClick(Sender: TObject);
begin
  aSelect.Execute;
end;

procedure TfmParts.gListDBTableViewKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  Case Key of
...
procedure TfmParts.gListDBTableViewStylesGetContentStyle(Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
  var AStyle: TcxStyle);
begin
