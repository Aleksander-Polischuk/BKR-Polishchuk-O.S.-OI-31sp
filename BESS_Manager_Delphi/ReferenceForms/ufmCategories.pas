...
type
  TfmCategories = class(TfmBaseListForm)
    gList: TcxGrid;
...
Uses ufmCategories_Ch, HomePage;
...
constructor TfmCategories.Create(AOwner: TComponent; Param: TStructure; Sender: TObject; AFormStyle: TFormStyle);
begin
...
procedure TfmCategories.aAddNewCopyExecute(Sender: TObject);
begin
  ...
  try
    ...
    mainModule.GetForm(TTypeForm.FormObj, DBTableName.REF_Categories, Param, Self, Sender, fsStayOnTop);
  finally
    Param.Free;
  end;
end;

procedure TfmCategories.aAddNewExecute(Sender: TObject);
begin
  var Param := TStructure.Create;
  try
...
procedure TfmCategories.aClearFindExecute(Sender: TObject);
begin
  edFind.Text := '';
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel).ApplyText('');
end;

procedure TfmCategories.aDeleteExecute(Sender: TObject);
var
  ID: Integer;
begin
  if spList.RecordCount = 0 then exit;
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_Categories', ID, false, true) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmCategories.aEditExecute(Sender: TObject);
begin
  if spList.RecordCount = 0 then exit;
...
procedure TfmCategories.aMarkDeleteExecute(Sender: TObject);
var
  ID: Integer;
  MarkDel: Boolean;
begin
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_Categories', ID, MarkDel, false) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmCategories.aRefreshExecute(Sender: TObject);
begin
  spList.Refresh;
end;

procedure TfmCategories.aSelectExecute(Sender: TObject);
begin
...
procedure TfmCategories.edFindPropertiesEditValueChanged(Sender: TObject);
begin
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel)
    .ApplyText(edFind.Text);
end;

procedure TfmCategories.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmCategories.gListDBTableViewDblClick(Sender: TObject);
begin
  aSelect.Execute;
end;

procedure TfmCategories.gListDBTableViewKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  Case Key of
...
procedure TfmCategories.gListDBTableViewStylesGetContentStyle(Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
  var AStyle: TcxStyle);
begin
