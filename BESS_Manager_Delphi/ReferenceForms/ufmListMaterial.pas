...
type
  TfmListMaterial = class(TfmBaseListForm)
    gList: TcxGrid;
    ...
    gListDBTableViewLENGTH: TcxGridDBColumn;
    ...
    gListDBTableViewIS_ROLL: TcxGridDBColumn;
...
var
  fmListMaterial    : TfmListMaterial;
  OwnSender : TObject;

implementation

Uses ufmListMaterial_Ch, HomePage;
...
constructor TfmListMaterial.Create(AOwner: TComponent; Param: TStructure; Sender: TObject; AFormStyle: TFormStyle);
begin
...
procedure TfmListMaterial.aAddNewCopyExecute(Sender: TObject);
begin
  ...
  try
    ...
    mainModule.GetForm(TTypeForm.FormObj, DBTableName.REF_MATERIALS, Param, Self, Sender, fsStayOnTop);
  finally
    Param.Free;
  end;
end;

procedure TfmListMaterial.aAddNewExecute(Sender: TObject);
begin
...
procedure TfmListMaterial.aClearFindExecute(Sender: TObject);
begin
  edFind.Text := '';
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel).ApplyText('');
end;

procedure TfmListMaterial.aDeleteExecute(Sender: TObject);
var
  ID: Integer;
begin
  if spList.RecordCount = 0 then exit;
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_MATERIALS', ID, false, true) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmListMaterial.aEditExecute(Sender: TObject);
begin
  if spList.RecordCount = 0 then exit;
...
procedure TfmListMaterial.aMarkDeleteExecute(Sender: TObject);
var
  ID: Integer;
  MarkDel: Boolean;
begin
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_MATERIALS', ID, MarkDel, false) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmListMaterial.aRefreshExecute(Sender: TObject);
begin
  spList.Refresh;
end;

procedure TfmListMaterial.aSelectExecute(Sender: TObject);
begin
...
procedure TfmListMaterial.edFindPropertiesEditValueChanged(Sender: TObject);
begin
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel)
    .ApplyText(edFind.Text);
end;

procedure TfmListMaterial.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmListMaterial.gListDBTableViewDblClick(Sender: TObject);
begin
  aSelect.Execute;
end;

procedure TfmListMaterial.gListDBTableViewStylesGetContentStyle(Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
  var AStyle: TcxStyle);
begin
