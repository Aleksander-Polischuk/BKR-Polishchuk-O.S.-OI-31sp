...
type
  TfmListMachine = class(TfmBaseListForm)
    gList: TcxGrid;
    ...
    gListDBTableViewPOWER_NOMINAL: TcxGridDBColumn;
    gListDBTableViewWORK_AREA_LENGTH: TcxGridDBColumn;
    ...
    gListDBTableViewWORK_AREA_WIDTH: TcxGridDBColumn;
    gListDBTableViewPOWER_IDLE: TcxGridDBColumn;
    gListDBTableViewDEFAULT_TOOL_RADIUS: TcxGridDBColumn;
    gListDBTableViewNOTES: TcxGridDBColumn;
    gListDBTableViewMAX_SPEED: TcxGridDBColumn;
...
var
  fmListMachine    : TfmListMachine;
  OwnSender : TObject;

implementation

Uses ufmListMachine_Ch, HomePage;
...
constructor TfmListMachine.Create(AOwner: TComponent; Param: TStructure; Sender: TObject; AFormStyle: TFormStyle);
begin
...
procedure TfmListMachine.aAddNewCopyExecute(Sender: TObject);
begin
  ...
  try
    ...
    mainModule.GetForm(TTypeForm.FormObj, DBTableName.REF_MACHINE, Param, Self, Sender, fsStayOnTop);
  finally
    Param.Free;
  end;
end;

procedure TfmListMachine.aAddNewExecute(Sender: TObject);
begin
...
procedure TfmListMachine.aClearFindExecute(Sender: TObject);
begin
  edFind.Text := '';
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel).ApplyText('');
end;

procedure TfmListMachine.aDeleteExecute(Sender: TObject);
var
  ID: Integer;
begin
  if spList.RecordCount = 0 then exit;
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_MACHINE', ID, false, true) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmListMachine.aEditExecute(Sender: TObject);
begin
  if spList.RecordCount = 0 then exit;
...
procedure TfmListMachine.aMarkDeleteExecute(Sender: TObject);
var
  ID: Integer;
  MarkDel: Boolean;
begin
  ...
  if mainModule.DelRecordSetMarkDel(spList, Self.Handle, 'REF_MACHINE', ID, MarkDel, false) then
    begin
      spList.Refresh;
    end;
end;

procedure TfmListMachine.aRefreshExecute(Sender: TObject);
begin
  spList.Refresh;
end;

procedure TfmListMachine.aSelectExecute(Sender: TObject);
begin
...
procedure TfmListMachine.edFindPropertiesEditValueChanged(Sender: TObject);
begin
  TcxGridFindPanelAccess(TcxGridTableControllerAccess(gListDBTableView.Controller).FindPanel)
    .ApplyText(edFind.Text);
end;

procedure TfmListMachine.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfmListMachine.gListDBTableViewDblClick(Sender: TObject);
begin
  aSelect.Execute;
end;

procedure TfmListMachine.gListDBTableViewStylesGetContentStyle(Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
  var AStyle: TcxStyle);
begin
